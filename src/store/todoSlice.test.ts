...
describe("todoSlice 단위 테스트", () => {
  ...
  it("deleteTodo로 todo를 삭제한다", () => {
    const initialState: TodoItem[] = [
      { id: "1", text: "삭제할 항목", completed: false },
    ];
  ...
  it("updateTodo로 text를 수정한다.", () => {
    const initialState: TodoItem[] = [
      { id: "1", text: "테스트 할 일", completed: false },
    ];
    const nextState = todoReducer(
      initialState,
      updateTodo({ id: "1", text: "수정된 할 일" })
    );

    expect(nextState[0].text).toBe("수정된 할 일");
    expect(nextState[0].completed).toBe(false); // 완료 상태는 그대로 유지
  });
});
